...
export function registerCombinedSearch(server: McpServer) {
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      // Ensure at least one search parameter is provided
      if (!query && !title && !author && !doi && !publicationYear && !yearRange) {
          ...
          (async () => {
            try {
              const params = {
                search: query || title,
                publicationYear,
                publicationYearRange: yearRange,
                isOa: isOpenAccess,
                perPage: maxResultsPerSource
              };

              const data = await openAlexSearch(params);
              results.push({
                source: 'OpenAlex',
                results: data
              });
        ...
        searchPromises.push(
          (async () => {
            try {
              ...
              const data = await crossrefSearch(params);
              results.push({
                source: 'Crossref',
                results: data
              });
